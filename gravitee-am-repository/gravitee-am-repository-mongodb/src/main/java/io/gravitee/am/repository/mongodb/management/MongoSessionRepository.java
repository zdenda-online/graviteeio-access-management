...
public class MongoSessionRepository extends AbstractManagementMongoRepository implements SessionRepository {
    ...
    public void init() {
        ...
        sessionCollection.createIndex(new Document(FIELD_RESET_TIME, 1), new IndexOptions().expireAfter(0L, TimeUnit.SECONDS)).subscribe(new LoggableIndexSubscriber());
    }
    ...
    public Single<Session> update(Session item) {
        return Single
                .fromPublisher(sessionCollection.replaceOne(eq(FIELD_ID, item.getId()), convert(item)))
